...
use crate::providers::usage::{ProviderId, ProviderUsage, UsageWindow, SESSION_WINDOW_MINUTES, WEEKLY_WINDOW_MINUTES, MONTHLY_WINDOW_MINUTES};

const USAGE_URL: &str = "https://opencode.ai/zen/go/v1/usage";

pub struct GoProvider { client: Client }
...
#[derive(Deserialize)]
struct Response { usage: Windows }
#[derive(Deserialize)]
struct Windows { rolling: Window, weekly: Window, monthly: Window }
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct Window { percent: f64, resets_at: String }

fn window(raw: Window, minutes: u32) -> Result<Option<UsageWindow>, UsageError> {
    let reset = chrono::DateTime::parse_from_rfc3339(&raw.resets_at).map_err(|_| UsageError::Parse)?.timestamp_millis();
    UsageWindow::new(raw.percent, minutes, Some(reset)).map(Some).ok_or(UsageError::Parse)
}
...
mod tests {
    ...
    fn maps_three_windows_with_exact_reset_datetimes() {
        ...
        assert_eq!(monthly.resets_at, Some(chrono::DateTime::parse_from_rfc3339("2026-10-08T12:00:00Z").unwrap().timestamp_millis()));
    }
    #[test]
    fn incomplete_or_wrong_contracts_do_not_become_zero_usage() {
        for body in [b"{}".as_slice(), b"<html>Sign in</html>", br#"{"usage":{"rolling":{"percent":1,"resetsAt":"bad"}}}"#] {
