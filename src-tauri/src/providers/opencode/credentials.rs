...
pub fn api_path() -> Result<PathBuf, CredentialError> {
    if let Some(data) = std::env::var_os("XDG_DATA_HOME").filter(|v| !v.is_empty()) {
        ...
        if path.is_absolute() {
            return Ok(path.join("opencode/auth.json"));
        }
    }
...
fn api_from_entries(
    ...
    path: &Path,
) -> Result<ApiCredential, CredentialError> {
    let missing = || CredentialError::MissingField {
        path: path.into(),
        field: "OpenCode Go API key",
    };
    let entry: Entry =
        serde_json::from_value(entries.get("opencode-go").cloned().ok_or_else(missing)?)
            .map_err(|_| missing())?;
    if entry.kind != "api" {
        return Err(missing());
    }
...
mod tests {
    ...
    fn api_credential_is_validated_and_redacted() {
        ...
        let credential = parse_api(
            r#"{"opencode-go":{"type":"api","key":"test-only-not-valid"}}"#,
            path,
        )
        .unwrap();
        ...
        for raw in [
            r#"{}"#,
            r#"{"opencode-go":{"type":"oauth","key":"fake"}}"#,
